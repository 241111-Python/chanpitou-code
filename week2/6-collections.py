...
print(nested_list)
print(
    sorted(nested_list[2])
)  # sorted returns a sorted list, but doesn't change the original order
...
usernames.discard(
    "dallas"
)  # discarding a value that is not part of the set will discard or not, without throwing an error

print("Romeo" in usernames)  # check for membership
print(usernames)
print(
    usernames.pop()
)  # pop returns a value from the set, and removes that entry from the set
print(usernames)
...
my_dictionary = {"rich": 33, "russell": 28, "andrew": 25}
